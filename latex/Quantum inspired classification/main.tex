...
\]
Because the JTC peak is proportional to the complex overlap \(\langle\psi\mid\phi\rangle\) of the optical fields, its squared magnitude directly supplies the fidelity of two pure optical encodings.
...
The only requirement is that the quantum centroids be \emph{nearly pure} (\(P(\varrho_k)\approx1\)); such high purity can be engineered through a judicious choice of the encoding that maps each training sample to an almost single–mode optical field whose decoherence is negligible over the correlation interval.
