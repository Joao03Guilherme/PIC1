...
\section{Machine learning and quantum-inspired classification}
...
\begin{figure}[htbp]
  ...
  \caption{Optics python simulation of the joint transform correlator with binary input images. This implementation uses the library LightPipes to simulate the optical propagation and Fourier transforms.}
\end{figure}
...
The binary joint transform correlator, introduced by B.Javidi1989 in \cite{Javidi1989}, 


\subsubsection{Binary Joint Transform Correlator}
...
\begin{table}[htbp]
  \centering
  \caption{Balanced Accuracy (mean $\pm$ std.) for multiple classifiers on the MNIST dataset. For the QNM-C classifier, we report results for different distance metrics and encodings. The best results are highlighted in bold.}
  \label{tab:cls-results}
  \begin{tabular}{@{}llcc@{}}
    \toprule
    Classifier & Metric & Encoding & Balanced Accuracy (\%)\\ 
    \midrule
    RBF-C & Euclidean & — & — \\
    RBF-C & JTC & — & — \\[2pt]
