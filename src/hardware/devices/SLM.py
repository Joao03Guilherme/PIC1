...
class SLMframe(wx.Frame):
    ...
    def __init__(self, monitor, isImageLock, alwaysTop):
        ...
        self.Window = SLMwindow(
            self, isImageLock=isImageLock, res=(self._resX, self._resY)
        )
        self.Show()
...
class Client:
    ...
    def _send_numpy_array(self, np_array):
        ...
        if self.compression == "bz2":
            data = bz2.compress(data, compresslevel=self.compression_level)
    ...
    def sendArray(self, arr: np.ndarray, timeout: float = 10, retries: int = 2):
        """
        Send a numpy array to the connected socket.
...
class SLMdisplay:
    ...
    def __init__(self, monitor=1, isImageLock=False, alwaysTop=False):
        self.isImageLock = isImageLock
    ...
    def listen_port(
        ...
        while True:
            ...
            print("Updating SLM")
            self.updateArray(image, sleep=0.0)
