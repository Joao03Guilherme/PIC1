...
def two_digits_phase_mask(d1, d2, display_scale=0.2):
    ...
    def mask(x_slm_coords, y_slm_coords, slm_physical_width, slm_physical_height):
        ...
        active_slm_region_mask = (
            (x_slm_coords >= x_display_start_slm)
            & (x_slm_coords < x_display_end_slm)
            & (y_slm_coords >= y_display_start_slm)
            & (y_slm_coords < y_display_end_slm)
        )
        ...
        if np.any(active_slm_region_mask):
            ...
            norm_x_for_bmp = (
                active_x_slm - x_display_start_slm
            ) / actual_display_width_on_slm
            norm_y_for_bmp = (
                active_y_slm - y_display_start_slm
            ) / actual_display_height_on_slm

            # Convert normalized coordinates to indices for the combined_bmp
            # combined_bmp has dimensions H_bmp (rows), W_bmp (columns)
            col_indices_bmp = np.clip(
                (norm_x_for_bmp * (W_bmp - 1)).astype(int), 0, W_bmp - 1
            )
            # y-axis for images is typically top-down, so (1 - norm_y_for_bmp) maps SLM's bottom-up y to image's top-down row.
            row_indices_bmp = np.clip(
                ((1 - norm_y_for_bmp) * (H_bmp - 1)).astype(int), 0, H_bmp - 1
            )

            # Assign phase values from combined_bmp to the active region of the output plane
            output_phase_plane[active_slm_region_mask] = combined_bmp[
                row_indices_bmp, col_indices_bmp
            ]
...
def second_pass(I1_original):  # Renamed to I1_original to avoid confusion
    ...
    I1_binary = (I1_original > threshold).astype(
        float
...
if __name__ == "__main__":
    ...
    p.add_argument(
        ...
        dest="plot3d",
        action="store_true",
        help="show 3D plot of correlation plane in a separate window",
    )
    p.add_argument(
        "--block-dc",
        type=float,
    ...
    im_input = ax_input.imshow(
        input_phase_image, extent=extent_mm, origin="lower", cmap="twilight_shifted"
    )

    # SLM Aperture
    rect_slm = plt.Rectangle(
        (-slm_w / (2 * mm), -slm_h / (2 * mm)),
        slm_w / mm,
        slm_h / mm,
        color="blue",
        fill=False,
        linestyle=":",
        linewidth=1,
        label=f"SLM ({slm_w/mm:.1f}x{slm_h/mm:.1f}mm)",
    )
    ax_input.add_patch(rect_slm)

    # Beam Waist
    # beam_waist is the 1/e^2 radius
    circle_beam = plt.Circle(
        (0, 0),
        beam_waist / mm,
        color="red",
        fill=False,
        linestyle="--",
        linewidth=1,
        label=f"Beam Waist ({beam_waist/mm:.1f}mm rad.)",
    )
    ...
    if args.plot3d:
        ...
        ax_3d_corr_plot = fig3d.add_subplot(
            1, 1, 1, projection="3d"
