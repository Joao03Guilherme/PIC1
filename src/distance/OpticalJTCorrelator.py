...
class OpticalJTCorrelator:
    ...
    def __init__(
        ...
        self.slm_width, self.slm_height = (
            self.slm.getSize()
        )  # Use more descriptive names

    def create_joint_input_plane(
        self, digit_array_ref: np.ndarray, digit_array_obj: np.ndarray, thresh=None
    ) -> np.ndarray:
        """
        ...
        def prepare_digit(digit_arr):
            if digit_arr.max() <= 1.0 and digit_arr.min() >= 0:  # Likely 0-1 float
                ...
                scaled = (
                    (digit_arr - digit_arr.min())
                    / (digit_arr.max() - digit_arr.min() + 1e-6)
                    * 255.0
                )
        ...
        img_resized_pil_combined = img_pil_combined.resize(
            (final_display_W, final_display_H), Image.BICUBIC
        )

        slm_canvas_arr = np.zeros(
            (slm_rows, slm_cols), dtype=np.float64
        )  # Use float for processing
        ...
        slm_canvas_arr[
            paste_y : paste_y + final_display_H, paste_x : paste_x + final_display_W
        ] = resized_combined_arr

        if self.binary_input:
            t_val = (
                np.median(slm_canvas_arr[slm_canvas_arr > 0])
                if thresh is None and np.any(slm_canvas_arr > 0)
                else (thresh if thresh is not None else 127)
            )
            # Ensure t_val is a scalar, not an array, if median of empty or all-zero is taken
            if not np.isscalar(t_val):
                t_val = 127
            ...
            if max_val > min_val:
                ...
                return np.full_like(
                    slm_canvas_arr,
                    int(min_val if min_val <= 255 else 0),
                    dtype=np.uint8,
                )
    ...
    def process_jps(self, jps_image: np.ndarray, thresh=None) -> np.ndarray:
        ...
        if self.binary_jps:
            ...
            if (
                jps_image.dtype == np.uint8
                and jps_image.min() == 0
                and jps_image.max() == 255
            ):
            ...
            if max_val > min_val:
                normalized = (
                    (jps_image.astype(np.float32) - min_val)
                    / (max_val - min_val)
                    * 255.0
                )
                return normalized.astype(np.uint8)
                ...
                return np.full_like(
                    jps_image, int(min_val if min_val <= 255 else 0), dtype=np.uint8
                )
    ...
    def set_display_scale(self, scale_factor: float):
        ...
        self.display_scale_factor = max(
            0.01, min(1.0, scale_factor)
