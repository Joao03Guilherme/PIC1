...
def main():
    ...
    shift_y, shift_x = IMAGE_SHAPE[0] // 4, IMAGE_SHAPE[1] // 4 # Shift by a fraction of image size
    ...
    try:
        ...
        _, _, _, corr_plane_shifted = classical_jtc(img_base_vec, img_shifted_vec, IMAGE_SHAPE)
        plot_filename_shifted = os.path.join(OUTPUT_DIR, "original_vs_shifted_corr_plane.png")
    ...
    try:
        _, _, _, corr_plane_random = classical_jtc(img_base_vec, img_random_vec, IMAGE_SHAPE)
        plot_filename_random = os.path.join(OUTPUT_DIR, "original_vs_random_corr_plane.png")
