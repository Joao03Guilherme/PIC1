...
pca = PCA(n_components=0.95, svd_solver='full', random_state=0) # Retain 95% of variance
...
print(f"Purity of the vector (from encoded vector): {purity_vector:.4f}") # Clarified print
...
for class_label in unique_classes:
    ...
    if len(class_samples) == 0:
        print(f"  Class {class_label}: No samples found, skipping centroid calculation.")
        continue
    ...
    if not class_density_matrices_list:
        print(f"  Class {class_label}: No density matrices generated, skipping centroid calculation.")
        continue
    ...
    if np.isclose(trace_val, 0):
        print(f"  Class {class_label}: Centroid trace is close to zero, cannot normalize. Purity will be calculated on unnormalized matrix.")
    ...
    centroids[class_label] = normalized_centroid_for_purity # Store the trace-normalized centroid
