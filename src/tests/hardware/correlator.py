...
def plot_images_and_correlation(
    digit1_vec,
    digit2_vec,
    shape,
    similarity,
...
def run_multi_correlation(correlator, digits_to_test=None, plot_all=False):
    ...
    summary_path = os.path.join(
        project_root, f"src/tests/hardware/optical_correlation_summary_{timestamp}.png"
    )
...
def main():
    ...
    if config["run_multi"]:
        ...
        print(
            f"Starting OpticalJTCorrelator test with digits {config['digit1']} and {config['digit2']}..."
        )
    ...
    try:
        ...
        print(
            f"Setting up OpticalJTCorrelator (sleep time: {config['sleep_time']}s)..."
        )
        correlator = OpticalJTCorrelator(
            slm=slm, cam=camera, sleep_time=config["sleep_time"]
        )
